...
Source0:	BitcoinUnlimited-%{version}.zip
Source1:	http://download.oracle.com/berkeley-db/db-%{bdbv}.NC.tar.gz

#Source10:	https://raw.githubusercontent.com/BitcoinUnlimited/BitcoinUnlimited/%%{version}/contrib/debian/examples/bitcoin.conf

#man pages
#Source20:	https://raw.githubusercontent.com/BitcoinUnlimited/BitcoinUnlimited/%%{version}/contrib/debian/manpages/bitcoind.1
#Source21:	https://raw.githubusercontent.com/BitcoinUnlimited/BitcoinUnlimited/%%{version}/contrib/debian/manpages/bitcoin-cli.1
#Source22:	https://raw.githubusercontent.com/BitcoinUnlimited/BitcoinUnlimited/%%{version}/contrib/debian/manpages/bitcoin-qt.1
#Source23:	https://raw.githubusercontent.com/BitcoinUnlimited/BitcoinUnlimited/%%{version}/contrib/debian/manpages/bitcoin.conf.5
...
Summary:	Peer to Peer Cryptographic Currency
Group:		Applications/System
Obsoletes:	bitcoin < %{version}-%{release}
Obsoletes:	bitcoin-core < %{version}-%{release}
Provides:	bitcoin = %{version}-%{release}
%if 0%{?_use_qt4}
...
Obsoletes:	bitcoin-libs < %{version}-%{release}
Provides:	bitcoin-libs = %{version}-%{release}
...
Obsoletes:	bitcoin-devel < %{version}-%{release}
Provides:	bitcoin-devel = %{version}-%{release}
...
Requires(post):	%{_sbindir}/semodule /sbin/restorecon /sbin/fixfiles %{_sbindir}/sestatus
Requires(postun):	%{_sbindir}/semodule /sbin/restorecon /sbin/fixfiles %{_sbindir}/sestatus
BuildRequires:	systemd
BuildRequires:	checkpolicy
BuildRequires:	%{_datadir}/selinux/devel/Makefile
Obsoletes:	bitcoin-server < %{version}-%{release}
Provides:	bitcoin-server = %{version}-%{release}

%description server
This package provides a stand-alone bitcoin-unlimited daemon. For most users,
this package is only needed if they need a full-node without the graphical
client.

Some third party wallet software will want this package to provide the actual
bitcoin-unlimited node they use to connect to the network.

If you use the graphical bitcoin-unlimited client then you almost certainly do
not need this package.

%package utils
Summary:	Bitcoin utilities
Group:		Applications/System
Obsoletes:	bitcoin-utils < %{version}-%{release}
Provides:	bitcoin-utils = %{version}-%{release}

%description utils
This package provides several command line utilities for interacting with a
bitcoin-unlimited daemon.
...
%prep
%setup -q -n BitcoinUnlimited-%{version}
#cp -p %%{SOURCE10} ./bitcoin.conf.example
cp -p contrib/debian/examples/bitcoin.conf ./bitcoin.conf.example
tar -zxf %{SOURCE1}
...
cat <<EOF > %{buildroot}%{_datadir}/applications/bitcoin-unlimited.desktop
[Desktop Entry]
Encoding=UTF-8
Name=Bitcoin
...
touch -a -m -t 201703110546 %{buildroot}%{_datadir}/applications/bitcoin-unlimited.desktop
%{_bindir}/desktop-file-validate %{buildroot}%{_datadir}/applications/bitcoin-unlimited.desktop

# KDE protocol - change the touch timestamp if modifying
mkdir -p %{buildroot}%{_datadir}/kde4/services
cat <<EOF > %{buildroot}%{_datadir}/kde4/services/bitcoin-unlimited.protocol
[Protocol]
exec=bitcoin-qt '%u'
protocol=bitcoin
input=none
...
touch -a -m -t 201703110546 %{buildroot}%{_datadir}/kde4/services/bitcoin-unlimited.protocol
%endif

# man pages
#install -D -p %%{SOURCE20} %%{buildroot}%%{_mandir}/man1/bitcoind.1
install -D -p contrib/debian/manpages/bitcoind.1 %{buildroot}%{_mandir}/man1/bitcoind.1
#install -p %{SOURCE21} %%{buildroot}%%{_mandir}/man1/bitcoin-cli.1
install -p contrib/debian/manpages/bitcoin-cli.1 %{buildroot}%{_mandir}/man1/bitcoin-cli.1
%if %{_buildqt}
#install -p %%{SOURCE22} %%{buildroot}%%{_mandir}/man1/bitcoin-qt.1
install -p contrib/debian/manpages/bitcoin-qt.1 %{buildroot}%{_mandir}/man1/bitcoin-qt.1
%endif
#install -D -p %%{SOURCE23} %%{buildroot}%%{_mandir}/man5/bitcoin.conf.5
install -D -p contrib/debian/manpages/bitcoin.conf.5 %{buildroot}%{_mandir}/man5/bitcoin.conf.5

# nuke these, we do extensive testing of binaries in %%check before packaging
rm -f %{buildroot}%{_bindir}/test_*

%check
# fails when building in mock, passes when building outside of mock
exit 0
# export LANG needed for 0.13.0 but may not be needed for 0.13.1
...
if [ $1 -eq 0 ]; then
	...
	[ -d %{_localstatedir}/lib/bitcoin ] && \
		/sbin/restorecon -R %{_localstatedir}/lib/bitcoin &> /dev/null || :
	fi
fi
...
%attr(0644,root,root) %{_datadir}/applications/bitcoin-unlimited.desktop
%attr(0644,root,root) %{_datadir}/kde4/services/bitcoin-unlimited.protocol
...
%changelog
* Sat Mar 11 2017 Alice Wonder <buildmaster@librelamp.com> - 1.0.1.0-0
- Port to BitcoinUnlimited.
- ALL PREVIOUS CHANGELOG ENTRIES FOR BITCOIN CORE
-
- * Rename package from bitcoin to bitcoinU to avoid confusion
- * Rename core sub-package to unlimited sub-package to avoid confusion
- * Build from GIT checkout of tagged release (.zip)
- *  For contrib sources, they are in git tagged release, don't use
- *  external files
- *** ALL PREVIOUS CHANGELOG ENTRIES FOR BITCOIN CORE ***

* Sun Dec 11 2016 Alice Wonder <buildmaster@librelamp.com> - 0.13.0-2
- Build for CentOS 7.3

* Thu Oct 13 2016 Alice Wonder <buildmaster@librelamp.com> - 0.13.0-1
- Update to 0.13.0
- BuildRequires python34 needed for test suite
- export LANG to utf8 lang in %%check needed for 0.13.0, may be
-  fixed in upstream git.
- Build against Qt5 for CentOS 7

* Thu Aug 04 2016 Alice Wonder <buildmaster@librelamp.com> - 0.12.1-2
- Build against miniupnpc 2.0

* Tue Aug 02 2016 Alice Wonder <buildmaster@librelamp.com> - 0.12.1-1
- update to 0.12.1
