...
void InitScriptExecutionCache()
{
    ...
    size_t nMaxCacheSize = std::min(std::max(int64_t(0), GetArg("-maxscriptcachesize", DEFAULT_MAX_SCRIPT_CACHE_SIZE)),
                               MAX_MAX_SCRIPT_CACHE_SIZE) *
                           (size_t(1) << 20);
    size_t nElems = scriptExecutionCache.setup_bytes(nMaxCacheSize);
    LOGA("Using %zu MiB out of %zu requested for script execution cache, "
