...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx, TransactionRecord *rec, int unit, QString labelFreeze)
{
    ...
    if (rec->type == TransactionRecord::PublicLabel)
    {
        ...
        std::string publicLabel = boost::replace_all_copy(rec->addresses.begin()->first,"\n"," ");

        strHTML = strHTML + "<b>" + tr("Public label:") + "</b> " + QString::fromStdString(publicLabel) + "<br>";

        std::vector<std::pair<CWalletTx, int>> listPublicLabels = wallet->GetPublicLabelTxs(rec->addresses.begin()->first);
        BOOST_FOREACH (PAIRTYPE(CWalletTx, int) plPair, listPublicLabels)
        {
            CWalletTx plTx = plPair.first;
            int i = plPair.second;
            //strHTML += "<b>" + tr("Date") + ":</b> " + (nTime ? GUIUtil::dateTimeStr(nTime) : "") + "<br>";
            strHTML += "<b>" + tr("Transaction ID") + ":</b> " + TransactionRecord::formatSubTxId(plTx.GetHash(), i) + "<br>";

            CTxDestination address;
            if (ExtractDestination(plTx.vout[i].scriptPubKey, address))
            {
                strHTML += "<b>" + tr("Address") + ":</b> ";
                strHTML += GUIUtil::HtmlEscape(CBitcoinAddress(address).ToString());

                if (wallet->IsMine(address) == ISMINE_SPENDABLE) strHTML += tr(" (own address)");

                strHTML += "<br>";

            }
            strHTML += "<b>" + tr("Unspent Amount") + ":</b> " +
                    BitcoinUnits::formatHtmlWithUnit(unit, plTx.vout[i].nValue) + "<br>";
            i++;
        }
    }
    else
        ...
        else
        {
            ...
            if (fAllFromMe)
            {
                ...
                BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                {
                    ...
                    {
                        // Include in description public label if it exists
                        std::string publicLabel = getLabelPublic(txout.scriptPubKey);
                        boost::replace_all(publicLabel,"\n"," ");
                        if (publicLabel != "")
                            strHTML = strHTML + "<b>" + tr("Public label:") + "</b> " + QString::fromStdString(publicLabel) + "<br>";
                        ...
                        if (publicLabel == "") // hide on public label txout
                        {
                            if(toSelf)
