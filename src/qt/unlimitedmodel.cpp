...
void UnlimitedModel::Init()
{
    ...
    std::string avg = QString::number(inUse ? aveKB : LLONG_MAX).toStdString();
    std::string burst = QString::number(inUse ? burstKB : LLONG_MAX).toStdString();
    ...
    avg = boost::lexical_cast<std::string>(inUse ? aveKB : LLONG_MAX);
    burst = boost::lexical_cast<std::string>(inUse ? burstKB : LLONG_MAX);
