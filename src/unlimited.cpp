...
// Just save the last few expedited sent blocks so we don't resend (uint256 zeros on construction)
...
bool HandleExpeditedBlock(CDataStream &vRecv, CNode *pfrom)
{
    ...
    if (msgType == EXPEDITED_MSG_XTHIN)
    {
        ...
        // If I have never seen the block or just seen an INV, treat the block as new
        bool newBlock = ((blkidx == NULL) || (!(blkidx->nStatus & BLOCK_HAVE_DATA)));
        // TODO replace with size of vRecv for efficiency
...
void UnlimitedSetup(void)
{
    ...
    {
        // This "leaks" in the sense that it is never freed, but is intended to last the duration of the program.
        new CStatHistory<uint64_t>("net/recv/msg/" + *i);
        // This "leaks" in the sense that it is never freed, but is intended to last the duration of the program.
        new CStatHistory<uint64_t>("net/send/msg/" + *i);
    }
