...
uint256 xpeditedBlkSent[NUM_XPEDITED_STORE]; // Just save the last few expedited sent blocks so we don't resend (uint256
...
std::string UnlimitedCmdLineHelp()
{
    ...
    strUsage += HelpMessageOpt("-maxlimitertxfee=<amt>",
        strprintf(
            _("Fees (in satoshi/byte) larger than this are always relayed (default: %s)"), DEFAULT_MAXLIMITERTXFEE));
    strUsage += HelpMessageOpt(
...
UniValue setminingmaxblock(const UniValue& params, bool fHelp)
{
    ...
    // size This also fixes issues user issues where people provide the value as MB
    if (arg < 100)
