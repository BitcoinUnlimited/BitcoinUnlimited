...
bool ContextualCheckBobtailBlockHeader(const CBlockHeader &block, CValidationState &state, CBlockIndex *const pindexPrev)
{
    ...
    if (block.GetBlockTime() <= pindexPrev->GetMedianBobtailTimePast())
...
bool InitBobtailBlockIndex(const CChainParams &chainparams)
{
    ...
    if (!fReindex)
    {
        ...
        catch (const std::runBobtailTime_error &e)
        {
    ...
    const bool fConservative)
{
    ...
    if (pindexPrev == nullptr)
        ...
        nLockTimeCutoff =
            (nLockTimeFlags & LOCKTIME_MEDIAN_TIME_PAST) ? pindexPrev->GetMedianBobtailTimePast() : block.GetBlockTime();
    ...
    std::vector<std::pair<uint256, CDiskTxPos> > &vPos)
{
    nFees = 0;
    int64_t nBobtailTime2 = GetStopwatchMicros();
    LOG(BLK, "Dependency ordering for %s MTP: %d\n", block.GetHash().ToString(), pindex->GetMedianBobtailTimePast());
    ...
    bool fParallel)
{
    ...
    if (pBestHeader)
    {
        if (fReindex || fImporting)
            fScriptChecks = !fCheckpointsEnabled || block.nBobtailTime > timeBarrier;
        else
            fScriptChecks = !fCheckpointsEnabled || block.nBobtailTime > timeBarrier ||
                ...
                for (int i = 0; i < BIT_WARNING_WINDOW && iindex != nullptr; i++)
                {
                    ...
                    if (!upgradedEval)
                    {
                        ...
                        int32_t nExpectedVersion =
                            UnlimitedComputeBlockVersion(pindex->pprev, chainParams.GetConsensus(), pindex->nBobtailTime);
    ...
    bool fParallel)
{
    ...
    LOCK(cs_blockvalidationBobtailTime);
    nBlockValidationBobtailTime << (end - start);
    return true;
}
