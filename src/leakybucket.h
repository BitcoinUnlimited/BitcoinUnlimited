...
static const int64_t DEFAULT_MAX_RECV_BURST = LLONG_MAX;
/** Default value for the maximum amount of data that can be sent in a burst */
static const int64_t DEFAULT_MAX_SEND_BURST = LLONG_MAX;
/** Default value for the average amount of data received per second */
static const int64_t DEFAULT_AVE_RECV = LLONG_MAX;
/** Default value for the average amount of data sent per second */
static const int64_t DEFAULT_AVE_SEND = LLONG_MAX;
...
public:
    CLeakyBucket(int64_t maxp, int64_t fillp, int64_t startLevel = LLONG_MAX) : max(maxp), fill(fillp)
    {
        lastFill = clock.now();
    ...
    {
        if (fill == LLONG_MAX)
            return LLONG_MAX; // shaping is off
        fillIt();
