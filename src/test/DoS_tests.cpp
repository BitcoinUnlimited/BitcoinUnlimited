...
// Helper function which will take a lock on cs_main before calling Misbehaving
// as a lock on cs_main must be held external to calls to Misbehaving
void MisbehavingHelper(NodeId pnode, int howmuch)
{
    LOCK(cs_main);
    Misbehaving(pnode, howmuch);
}
...
BOOST_AUTO_TEST_CASE(DoS_banning)
{
    ...
    MisbehavingHelper(dummyNode1.GetId(), 100); // Should get banned
    ...
    MisbehavingHelper(dummyNode2.GetId(), 50);
...
BOOST_AUTO_TEST_CASE(DoS_banscore)
{
    ...
    CNode dummyNode1(INVALID_SOCKET, addr1, "", true);
    dummyNode1.nVersion = 1;
    MisbehavingHelper(dummyNode1.GetId(), 100);
    SendMessages(&dummyNode1);
    BOOST_CHECK(!CNode::IsBanned(addr1));
    MisbehavingHelper(dummyNode1.GetId(), 10);
    SendMessages(&dummyNode1);
    BOOST_CHECK(!CNode::IsBanned(addr1));
    MisbehavingHelper(dummyNode1.GetId(), 1);
    SendMessages(&dummyNode1);
    BOOST_CHECK(CNode::IsBanned(addr1));
...
BOOST_AUTO_TEST_CASE(DoS_bantime)
{
    ...
    MisbehavingHelper(dummyNode.GetId(), 100);
