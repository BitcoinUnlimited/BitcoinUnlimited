...
class SchnorrActivationTest(BitcoinTestFramework):
    ...
    def set_test_params(self):
        ...
        self.tip = None
        self.blocks = {}
        self.extra_args = [['-whitelist=127.0.0.1',"-consensus.forkMay2019Time={}".format(MAY_2019_FORK_TIME)],
    ...
    def get_tests(self):
        ...
        # activation.
        bfork = block(5555, nTime=MAY_2019_FORK_TIME - 1)
        yield accepted()

        sync_blocks(self.nodes)

        # Create 5 more blocks with timestamps from MAY_2019_FORK_TIME+0 to +4
        for i in range(5):
            block(5200 + i)
            test.blocks_and_transactions.append([self.tip, True])
        yield test
